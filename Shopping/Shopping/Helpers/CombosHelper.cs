...
namespace Shopping.Helpers
{
    public class CombosHelper : ICombosHelper
    {
        ...
        public CombosHelper(DataContext context)//inyeccion del datacontext
        {
            _context = context;
        }
        ...
        {
            List<SelectListItem> list = await _context.Categories.Select(c => new SelectListItem//se hace la conversion de lista de categorias a listitem
            {
                Text = c.Name,
                Value = c.Id.ToString()
            })
                .OrderBy(c => c.Text)//ordenar 
                .ToListAsync();
        ...
        public async Task<IEnumerable<SelectListItem>> GetComboCitiesAsync(int stateId)
        {
            List<SelectListItem> list = await _context.Cities
                .Where(s => s.State.Id == stateId)//se filtra las ciudades que pertenescan al estado
