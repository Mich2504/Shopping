...
namespace Shooping.Controllers
{
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                     ModelState.AddModelError(string.Empty, "Ha superado el maximo numero de intentos, su cuenta " +
                         "esta bloqueada, intente de nuevo en 5 minutos");
                    //_flashMessage.Danger("Ha superado el máximo número de intentos, su cuenta está bloqueada, intente de nuevo en 5 minutos.");
                }
                else if (result.IsNotAllowed)//ya se registro pero no ha confirmado el correo
                {
        ...
        {
            if (ModelState.IsValid)//Esta linea es de suma importancia ya que ayuda a que no se genere tanto codigo
            {
                ...
                //ya no quiero que se el usuario haga login
    ...
    //Realiza la confrimacion del email
